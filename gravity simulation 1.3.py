...
# mejorar lógica para secuencia de creación de planeta(radio y posicion) y vector (dirección y velocidad) HECHO
#permitir ver magnitud (velocidad) en la etapa de creación de vector #HECHO
#en la creación de planetas, añadir vector que indique velocidad y direccion HECHO
...
def showVectorCalculation(i):
    ...
    modulo = math.sqrt((dx - x_centro)**2 + (dy - y_centro)**2)
    
    x_final = x_centro + dx * modulo * 0.007
    y_final = y_centro + dy * modulo * 0.007
    
    showVector(x_centro, y_centro, x_final, y_final)
...
text5 = font.render("Presionar 'V' para mostrar vectores de velocidad y dirección",True,(255,255,255))
...
def showInfo():
    ...
    disp.blit(text1, (x+10, y+10))
    disp.blit(text2, (x+10, y+20))
    disp.blit(text3, (x+10, y+30))
    disp.blit(text4, (x+10, y+40))
    disp.blit(text5, (x+10, y+50))
    disp.blit(text6, (x+10, y+60))
...
def drawTempPlanet(xDown,yDown,xStillDown,yStillDown):
    ...
    p = planet(xDown,yDown,0,radio,150,150,150,"name")#crea un planeta sin masa y luego de finalizar este proceso, le añade masa y lo apendiza a la lista de planetas. del mismo modo dibuja un circulo que no será el planeta asi no es afectado por la gravedad de otros planetas mientras se dibuja.
...
while state:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or quitKey:
            state = False
        ...
        # Bloque de creación
        if not quitCreatingKey:
            if event.type == pygame.MOUSEBUTTONDOWN: #Si pulso el click y !createKey
